<?php
/*
+ ----------------------------------------------------------------------------+
|     e107 website system - Dutch Language File.
|
|     $URL:$
|     $Revision:$
|     $Date:$
|     $Author: Yvalni, Tgtje $
+----------------------------------------------------------------------------+
*/
return [

    'BNRLAN_00' => "Nog geen banners aangemaakt.",//15
    'BNRLAN_01' => "Banner ID niet gevonden.",
//'BNRLAN_7' => "Statistieken", 
//'BNRLAN_8' => "NVT",
//'BNRLAN_9' => "Start", 
//'BNRLAN_10' => "Eindigd", 
    'BNRLAN_11' => "Campagne",
    'BNRLAN_12' => "Klant login",
//'BNRLAN_13' => "Klant wachtwoord",
//'BNRLAN_14' => "Banner afbeelding", 
    'BNRLAN_15' => "klik URL",
    'BNRLAN_16' => "Aangeschafte doorkliks",
//'BNRLAN_17' => "Startdatum", 
//'BNRLAN_18' => "Einddatum", 
// 19 - 24 extra space
    'BNRLAN_25' => "Voeg nieuwe of een bestaande campagne toe",
    'BNRLAN_28' => "Voeg nieuwe of een bestaande klant toe",
    'BNRLAN_29' => "Voeg nieuwe klant toe",
//'BNRLAN_30' => "Nieuwe klant", 
    'BNRLAN_31' => "Onbeperkt indien leeg gelaten.",
    'BNRLAN_32' => "Kies banner afbeelding",
    'BNRLAN_33' => "Code",
// 34 - 35 extra space
    'BNRLAN_35' => "Banner menu",
//'BNRLAN_36' => "Banner menu<br> configuratie", 
//'BNRLAN_37' => "Hoofdtitel", 
    'BNRLAN_38' => "Advertentie",
    'BNRLAN_39' => "kies de in het menu te tonen campagnes",
    'BNRLAN_40' => "Nog geen campagnes.",
    'BNRLAN_41' => "Aantal te tonen banners",
    'BNRLAN_42' => "dit geldt alleen als er meerdere campagnes zijn geselecteerd",
    'BNRLAN_43' => "Hoe de campagnes te tonen?",
    'BNRLAN_44' => "Weergave type..",
    'BNRLAN_45' => "Een campagne in een enkel menu",
    'BNRLAN_46' => "Alle geselecteerde campagnes in een enkel menu",
    'BNRLAN_47' => "Alle geselecteerde campagnes in afzonderlijke menus",
    'BNRLAN_48' => "Alle geselecteerde campagnes (geen box rendering)",
    'BNRLAN_50' => "Lokaal",
    'BNRLAN_51' => "Op afstand",
    'BNRLAN_HELP_IMPRESSIONS' => "0 = Onbeperkt",
    'BNRLAN_HELP_KEYWORDS' => "Wanneer nieuws of pagina's worden geladen, beperkt dit de bannerresultaten tot overeenkomende zoekwoorden. Wees hier voorzichtig mee.",
    'BNRLAN_HELP_01,'=> "Banner Hulp";
    'BNRLAN_HELP_02' => "Gebruik deze plugin om advertenties op je website te beheren.[br][br]Met het opmaken van een nieuwe banner kan je een gebruikersnaam en wachtwoord aanmaken voor je klant om in te loggen en de voortgang van de campagne te volgen.[br][br]Je kan de banner activeren en weergeven via het menu beheer en via die pagina verder configureren.",
    'BNRLAN_TITLE_REMOTE_IMAGE' => "Dit overschrijft elke lokale afbeelding die je hebt ingesteld.",
    'BNRLAN_MENU_MANAGER_CONFIG' => "Het menu is nu geconfigureerd in de menu-manager.",
    'BNRLAN_PLACEHOLDER_OR_NEW_CLIENT' => "Of voer nieuwe client in ",
    'BNRLAN_PLACEHOLDER_ENTER_CAMPAIGN' => "Geef een capmpagne naam in ",
];
