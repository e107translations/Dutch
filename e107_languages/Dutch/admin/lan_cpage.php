<?php
/*
+ ----------------------------------------------------------------------------+
|     e107 website system - Dutch Language File.
|
|     $URL:$
|     $Revision:$
|     $Date:$
|     $Author: Yvalni, Tgtje $
+----------------------------------------------------------------------------+
*/
define("CUSLAN_1", "Overzicht");
define("CUSLAN_2", "Pagina titel");
define("CUSLAN_3", "Items per pagina");
define("CUSLAN_4", "Custom velden");
define("CUSLAN_5", "(Nieuw boek)");
define("CUSLAN_9", "Tekst");
//define("CUSLAN_11", "Meta omschrijving");
define("CUSLAN_12", "Aanmaken pagina/menu");
define("CUSLAN_29", "Overzicht pagina's als geen geselecteerd");
define("CUSLAN_30", "Vervalperiode cookie (in seconden)");
define("CUSLAN_31", "Aanmaken menu");
define("CUSLAN_48", "Pagina overzicht");
define("CUSLAN_49", "Menu overzicht");
define("CUSLAN_50", "Toon boeken/hoofdstukken");
define("CUSLAN_51", "Voeg boek/hoofdstuk toe");
define("CUSLAN_52", "Boek");
define("CUSLAN_53", "Titel boek of hoofdstuk");
define("CUSLAN_55", "Kan bewerkt worden door");
define("CUSLAN_56", "Ouder");
define("CUSLAN_57", "Kies aub een unieke SEF URL voor deze toevoeging");
define("CUSLAN_58", "Bekijk pagina\"s in dit Hoofdstuk");
define("CUSLAN_59", "Pagina");
define("CUSLAN_60", "Pagina Opties");
define("CUSLAN_61", "Menu");
define("CUSLAN_62", "Menu Opties");
define("CUSLAN_63", "Boek/Hoofdstuk");
define("CUSLAN_64", "Menu Naam");
define("CUSLAN_65", "Menu Titel");
define("CUSLAN_66", "Menu Inhoud");
define("CUSLAN_67", "Menu Template");
define("CUSLAN_68", "Maatwerk Knop Tekst");
define("CUSLAN_69", "Maatwerk Knop URL");
define("CUSLAN_70", "Menu Icoon/Glyph");
define("CUSLAN_71", "Menu Afbeelding/Video");
define("CUSLAN_72", "Toon Boeken/Hoofdstukken Template");
define("CUSLAN_73", "Menu aangemaakt");
define("CUSLAN_74", "Menu bijgewerkt");
define("CUSLAN_75", "Missend Menu-id ontdekt:");
define("CUSLAN_76", "Menu met pad #");
define("CUSLAN_77", "verwijderd");
define("CUSLAN_78", "Kon menu met pad niet verwijderen");
define("CUSLAN_79", "Je dient een pagina titel of een menu naam in te geven.");
define("CUSLAN_80", "Ondertitel"); //sub title
define("CUSLAN_81", "Meta afbeelding");
define("CUSLAN_82", "Optioneel. Gebruikt door sociale media bij het delen van deze pagina. ");
define("CUSLAN_83", "Wordt onder deze naam weergegeven in de Menu-Manager of kan worden aangeroepen met {CMENU=name} in uw thema. Mag alleen ASCII-tekens gebruiken en mag alleen uit kleine letters bestaan.");
define("CUSLAN_84", "Titel weergegeven bij het menu-item.");
define("CUSLAN_85", "Laat dit leeg om de standaardwaarde te gebruiken");
define("CUSLAN_86", "Laat dit leeg om de overeenkomstige pagina te gebruiken");
define("CUSLAN_87", "Pagina's aanwezig"); //weergave aantal